  ...
  useEffect(() => {
    ...
    if (serviceParam) {
      for (const category of serviceCategories) {
        const match = category.options.find((opt) =>
...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  try {
    const res = await fetch("/api/whatsapp-verification", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        name,
        phone: phone.startsWith("+91") ? phone : `+91${phone}`,
        guests: 1, // You can update this if you have a guests field
    ...
    setErrorMsg(err.message || "Network error. Try again later.")
  } finally {
